...
class CardyfieWithdrawFundController extends GetxController {
  ...
  void onInit() {
    ...
    amountTextController.addListener(() => calculation());
  }
  ...
  Future<CommonSuccessModel?> cardWithdrawProcess(BuildContext context) async {
    Map<String, dynamic> inputBody = {
      'card_id': controller.cardyfieCardUIId.value,
      'withdraw_amount': int.parse(amountTextController.text.trim()),
    ...
    return RequestProcess().request<CommonSuccessModel>(
      ...
      apiEndpoint: ApiEndpoint.cardyfieWithdrawCard,
      ...
      onSuccess: (value) {
        ...
        StatusScreen.show(
          context: context,
          subTitle: Strings.yourmoneyWithdrawSuccess,
          onPressed: () {
  ...
  void calculation() {
    ...
    percentCharge.value = double.parse(data.cardWithdrawPercentFee.toString());
    depositCharge.value =
        double.parse(data.cardDepositFixedFee.toString()) * exchangeRate.value;

    totalCharge.value =
        (double.parse(data.cardWithdrawFixedFee.toString()) *
            exchangeRate.value) +
