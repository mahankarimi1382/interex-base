...
import 'package:qrpaypro/backend/model/pusher/pusher_beams_model.dart';
import 'package:qrpaypro/backend/services/api_endpoint.dart';
import 'package:qrpaypro/backend/services/pusher/pusher_api_services.dart';
    ...
    await ApiServices.dashboardApi()
        .then((value) async {
          ...
          if (data.moneyExchange) {
            categoriesData.add(
              CategoriesModel(
                Assets.icon.exchangeAlt,
                Strings.moneyExchange,
                () {
          ...
          if (data.myChat) {
            categoriesData.add(
              CategoriesModel(Assets.icon.myChat, Strings.myChat, () {
                Get.toNamed(Routes.myChatScreen);
              }),
            );
          }
          ...
          if (data.trade) {
            categoriesData.add(
              CategoriesModel(Assets.icon.trade, Strings.p2pTrade, () {
                Get.toNamed(Routes.tradeScreen);
              }),
            );
          }
          ...
          percentCharge.value =
              _dashboardModel.data.cardReloadCharge.percentCharge;
  ...
  Future<PusherBeamsModel> getPusherAuth() async {
    ...
    await PusherApiServices.getPusherBeamsAuth(LocalStorages.getId()!)
        .then((value) {
          ...
          getSecure();
          LocalStorages.savePusherAuthenticationKey(
