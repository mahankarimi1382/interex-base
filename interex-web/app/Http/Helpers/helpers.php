        ...
        if (array_shift($file_mime_type_array) == 'image' && $file_extension != 'svg') { // If Image
            ...
            if ($width > 0 && $width < 2048) {
                ...
                try {
                    ...
                    $image_encode = $file->scale(width: $new_width)->encodeByExtension($file_extension, quality: 70);
                } catch (Exception $e) {
            ...
            if (isset($image_encode)) {
                Storage::disk(Storage::getDefaultDriver())->put($path, (string) $image_encode, [
                    'visibility' => 'public',
                ]);
            }
        ...
        if (array_shift($file_mime_type_array) == 'image' && $file_extension != 'svg') { // If Image
            ...
            if ($crop === true) {
                ...
                if ($crop_width != false && $crop_height != false) {
                    $file->cover($crop_width, $crop_height, $crop_position);
                }

                if ($crop_width != false && $crop_height == false) {
                    $file->scale(width: $crop_width);
                }
            }

            // Save File
            try {
                $image_encode = $file->encodeByExtension($file_extension, quality: 70);
            } catch (Exception $e) {
...
{
    return rtrim(Storage::disk(Storage::getDefaultDriver())->url(''), '/');
}
