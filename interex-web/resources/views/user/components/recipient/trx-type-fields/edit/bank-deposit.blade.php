        ...
        <div class="col-xl-4 col-lg-4 col-md-6 form-group">
            ...
            <select name="country" class="form--control country-select select2-basic">
                ...
                @foreach ($countries as $item)
                    <option value="{{ $item->id }}" {{ @$data->country == $item->id ?'selected':''}} data-country-code="{{ $item->code }}" data-mobile-code="{{ get_country_phone_code($item->country)}}"  data-id="{{ $item->id }}">{{ $item->name }} ({{ $item->code }})</option>
                @endforeach
            </select>
        </div>
